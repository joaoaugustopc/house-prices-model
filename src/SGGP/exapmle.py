...
# Training samples
X_train = rng.uniform(-1, 1, 1000).reshape(100, 10)
y_train = X_train[:, 0]**2 - np.sin(X_train[:, 1]) + X_train[:, 1] - 1

# Testing samples
X_test = rng.uniform(-1, 1, 1000).reshape(100, 10)
y_test = X_test[:, 0]**2 - np.sin(X_test[:, 1]) + X_test[:, 1] - 1
est_gp = SymbolicRegressor(population_size=500,generations=1000, stopping_criteria=1e-5,
